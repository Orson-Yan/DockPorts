...
| 🐳 **Docker 镜像** | [`orsoman/dockports`](https://hub.docker.com/r/orsoman/dockports)（由本仓库构建发布） |

> 本仓库 [Orson-Yan/DockPorts](https://github.com/Orson-Yan/DockPorts) 是 `orsoman/dockports` 镜像的**唯一构建来源**，请以此仓库为准。
> 项目最初 Fork 自 [coracoo/DockPorts](https://github.com/coracoo/DockPorts)，在其基础上大量重构与新增功能，感谢原作者的开源贡献。
...
- 🌐 **暴露面分析**：识别每个端口的绑定范围——公开（`0.0.0.0`/`::`）/ 局域网（具体网卡）/ 仅本机（回环），一眼看清哪些端口真的对外开放，可按暴露面筛选
- 🩺 **主动探活**：可选对 TCP 端口做连接探测，显示「🟢 通 / 🔴 不通」，区分「在监听」与「真可用」（设置内开启）
...
### v1.0.0 🎉 首个稳定版本
- 🌐 **端口暴露面分析**：采集每个端口绑定 IP，分级 公开 / 局域网 / 仅本机，卡片显示彩色徽章（hover 看绑定 IP），工具栏支持按暴露面筛选，CSV 导出含暴露面列
- 🩺 **端口主动探活**：设置内开启后对 TCP 端口做连接探测（按实际绑定 IP，避免误报），卡片显示「🟢 通 / 🔴 不通」，并发探测 + 15 秒缓存
- 📌 明确仓库与镜像关系：本仓库为 `orsoman/dockports` 镜像的唯一构建来源
- 汇总此前 0.3.x 的功能与加固（预留视图、占用进程、鉴权加固、分类修复、卡片重排等），标志功能趋于稳定
